...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "missing capability — generowanie propozycji treningu na bazie własnej historii"
    - topic: "primary persona"
      decision: "hobbyista trenujący siłowo amatorsko"
    - topic: "insight"
      decision: "prostota (brak przeładowania jak w Hevy/Strong) + propozycje bazujące na własnych danych historycznych + kontrola nad własną bazą ćwiczeń"
  frs_drafted: 0
...
Insight: 10xBoar wygrywa trzema rzeczami jednocześnie — prostotą obsługi (trening dodany w < 1 minucie), propozycjami generowanymi na podstawie własnych danych historycznych (nie gotowych szablonów) oraz pełną kontrolą nad własną bazą ćwiczeń. Żadna prosta aplikacja nie łączy tych trzech cech.
...
- Dane treningowe użytkownika nie są dostępne dla żadnego innego konta — izolacja danych per użytkownik jest egzekwowana na poziomie każdego żądania.
...
1. **Jaka minimalna liczba zalogowanych sesji jest wymagana, zanim aplikacja wygeneruje sensowną propozycję?** — Do ustalenia przed implementacją algorytmu. Blokuje: tak (propozycja z zerową historią szkodzi zaufaniu).
