...
```bash
npx astro sync     # regenerate astro:env / astro:content types (run after changing astro.config.mjs)
```

No test suite is configured — CI runs `lint` + `build` only.

For other commands see @README.md

## Architecture

Full SSR app (`output: "server"`) deployed to Cloudflare Workers via `@astrojs/cloudflare`. React is used as an island framework for interactive components only; layout and page shells are plain Astro.

### Key conventions

- **Supabase client**: Always created per-request via `createClient(request.headers, cookies)` from `@/lib/supabase`. It returns `null` when env vars are missing — callers must handle that case.
- **Env vars**: Loaded via `astro:env/server` (declared in `astro.config.mjs` `env.schema`), not `process.env`. Adding a new secret requires updating the schema in `astro.config.mjs` before it can be imported.
- **Local secrets**: The Cloudflare dev runtime reads `.dev.vars`, not `.env`. Copy `.env.example` to `.dev.vars` for local development (`cp .env.example .dev.vars`).
- **Path alias**: `@/` resolves to `src/`. Always import with `@/`, never with relative paths across feature areas.
- **Auth flow**: Auth forms POST to `/api/auth/{signin,signup,signout}`. On error, the API route redirects back with `?error=<encoded message>`; on success it redirects to the destination page. There is no JSON API for auth.
- **Route protection**: Add paths to the `PROTECTED_ROUTES` array in `src/middleware.ts`. The middleware populates `context.locals.user` (typed in `src/env.d.ts`) for all routes.
...
Pre-commit hook runs automatically: ESLint `--fix` on `*.{ts,tsx,astro}`, Prettier on `*.{json,css,md}`.
